...
export function OvertimeSheet({ open, onClose }: OvertimeSheetProps) {
  ...
  const submit = useMutation({
    mutationFn: (data: FormData) => employeeApi.submitOvertimeRequest(data),
    onSuccess: () => {
      setSuccess(true);
  ...
  return (
    <BottomSheet open={open} onOpenChange={(v) => !v && handleClose()}>
      <BottomSheetContent title="Overtime Request">
        {success ? (
          <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">Your overtime request is pending approval.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit((d) => submit.mutate(d))} className="px-5 pb-8 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">Overtime Hours</label>
              <Input
                type="number"
                step="0.5"
                min="0.5"
